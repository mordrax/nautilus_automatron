...
export const useCatalogIndicators = (barType: string) => {
  ...
  const { data } = useQuery({
    queryKey: ['catalog-indicator-data', barType, sortedIds],
    queryFn: () => api.runEffect(api.getCatalogIndicatorResult(barType, sortedIds)),
